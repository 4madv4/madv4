	...
	function whitelist:update(first)
		local suc = pcall(function()
			local _, subbed = pcall(function()
				return game:HttpGet('https://github.com/4madvape/whitelistcheck')
			end)
			local commit = subbed:find('currentOid')
			commit = commit and subbed:sub(commit + 13, commit + 52) or nil
			commit = commit and #commit == 40 and commit or 'main'
			whitelist.textdata = game:HttpGet('https://raw.githubusercontent.com/4madvape/whitelistcheck/'..commit..'/PlayerWhitelist.json', true)
		end)
	...
	ChatSpammer = vape.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				local ind = 1
				repeat
					local message = (#Lines.ListEnabled > 0 and Lines.ListEnabled[math.random(1, #Lines.ListEnabled)] or 'madvape on top')
